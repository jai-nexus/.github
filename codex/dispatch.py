...
Env:
  ...
  GITHUB_API              -> override API base if needed (default: "https://api.github.com")
...
def get_installation_id(app_jwt: str) -> int:
    r = requests.get(f"{GITHUB_API}/orgs/{ORG}/installation",
...
def get_installation_token(app_jwt: str, inst_id: int) -> str:
    ...
    # Ask for Actions:write so we can dispatch workflows.
    # If the installation hasn't accepted updated perms yet, fall back to defaults.
    body = {"permissions": {"actions": "write", "contents": "read"}}
    r = requests.post(url, headers=hdr, json=body)
    if r.status_code == 422:
        r = requests.post(url, headers=hdr)  # fallback to granted defaults
...
def list_workflows(inst_token: str) -> Dict[str, Any]:
    ...
    r = requests.get(url, headers=_hdr_token(inst_token))
    if r.status_code >= 300:
...
def main():
    ...
    if args.cmd == "check":
        ...
        names = [w.get("name", w.get("path", "")) for w in data.get("workflows", [])]
        print(f"Workflows ({len(names)}): {', '.join(names)}")
        return
